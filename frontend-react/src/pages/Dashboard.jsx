...
import {
  RefreshCw, TrendingUp, ArrowDownCircle, PlusCircle,
    ...
    <div className="flex flex-col items-center justify-center py-20 gap-5 animate-fade-up">
      ...
      <div className="text-center max-w-sm">
        ...
        <p className="text-[0.8rem] text-slate-500 leading-relaxed">
          Registre entradas e saídas para o dashboard mostrar seus números reais.
        </p>
      </div>
  ...
  return (
    <div className="card p-5 animate-fade-up flex flex-col">
      <div className="flex items-center justify-between mb-4">
        ...
        <a href="#agenda" className="text-[0.68rem] font-semibold text-blue-600 hover:underline">Ver agenda →</a>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center py-5">
          ...
          <a href="#agenda" className="mt-2 text-[0.72rem] text-blue-500 font-medium hover:underline">Criar evento</a>
        </div>
      ) : (
            ...
            return (
              <div key={ev.id} className="flex items-center gap-2.5">
                ...
                <div className="text-right shrink-0">
                  <p className="text-[0.67rem] font-bold" style={{ color: cfg.color }}>{fmtDateShort(ev.date)}</p>
...
export default function Dashboard() {
  ...
  const kpiData = data || { entradas:0, saidas:0, saldo:0, acumulado:0 }

  // Dados reais dos gráficos — arrays vazios quando não há dados (sem mock)
  const monthly      = data?.monthly      || []
  const saidas_cats  = data?.categories   || []
  const receita_tipos = data?.receita_tipos || []

  // Projetos: reais se disponíveis
  const projetos = (data?.projects?.list?.length
    ? data.projects.list.map(p => ({ name: p.name, margem: p.margin, receita: p.revenue, custo: p.cost }))
    : []
  )
  ...
  return (
    <div className="p-7 space-y-5">
      ...
      {/* Gráficos — só exibe quando há dados reais */}
      {!loading && hasData && (
        <>
          <div className="grid grid-cols-5 gap-4">
            <ChartCard title="Entradas vs Saídas" subtitle="Mês a mês no ano" className="col-span-3"
            ...
            <div className="card p-5 animate-fade-up flex flex-col">
              ...
              {projetos.length === 0 ? (
                <div className="flex-1 flex items-center justify-center">
                  <p className="text-[0.75rem] text-slate-300">Nenhum projeto ativo</p>
                </div>
              ) : (
                      ...
                      <div key={p.name}>
                        ...
                        <div className="h-1.5 rounded-full overflow-hidden" style={{ background:barBg }}>
                          <div className="h-full rounded-full transition-all duration-700"
                            style={{ width:`${Math.min(m,100)}%`, background:`linear-gradient(90deg,${bar}99,${bar})` }} />
                        </div>
                      </div>
                    )
                  })}
              ...
              <div className="mt-4 pt-3.5 space-y-1.5" style={{ borderTop:'1px solid rgba(226,232,240,0.7)' }}>
                <div className="flex justify-between text-[0.75rem]">
                  ...
                  <span className="font-bold text-emerald-600">{fmtBRL(projetos.reduce((s,p)=>s+(p.receita||0),0))}</span>
                </div>
                <div className="flex justify-between text-[0.75rem]">
                  <span className="text-slate-500">Custo total</span>
                  <span className="font-bold text-red-500">{fmtBRL(projetos.reduce((s,p)=>s+(p.custo||0),0))}</span>
                </div>
              </div>
            </div>
